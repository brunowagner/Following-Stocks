...
struct HTTPTools {
    ...
    static func treatsDataTask (data : Data?,response: URLResponse?,error : Error?, apiRequirements : ApiRequirements, completionHandlerForTreatsDataTask: @escaping (_ result: AnyObject?, _ error: NSError?) -> Void) {
        ...
        guard let statusCode = (response as? HTTPURLResponse)?.statusCode else{
            sendError(errorCode: Errors.ErrorCode.Response_statusCode_error.rawValue, errorString: "Your request returned a invalid status code!")
            return
        }
        ...
        guard let data = data else {
            sendError(errorCode: Errors.ErrorCode.No_data_or_unexpected_data_was_returned.rawValue ,errorString: "No data was returned by the request!")
            return
        }
    ...
    private static func convertDataWithCompletionHandler(_ data: Data, completionHandlerForConvertData: (_ result: AnyObject?, _ error: NSError?) -> Void) {
        ...
        do {
            ...
            completionHandlerForConvertData(nil, NSError(domain: "convertDataWithCompletionHandler", code: Errors.ErrorCode.Could_not_parse_the_data.rawValue, userInfo: userInfo))
        }
